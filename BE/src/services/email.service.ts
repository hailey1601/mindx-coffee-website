...
export const sendEmail = async (to: string, subject: string, html: string) => {
  try {
    ...
    console.warn('⚠️ [SMTP Error] Email delivery failed:', error.message);
    console.log('-----------------------------------------');
    console.log(`✉️ Mock email sent to: ${to}`);
    console.log(`✉️ Subject: ${subject}`);
    // Extract OTP code or token from HTML if present
    const otpMatch = html.match(/>(\d{6})<\/span>/) || html.match(/<b>(\d{6})<\/b>/);
    if (otpMatch) {
      console.log(`🔑 Verification/Reset Code: ${otpMatch[1]}`);
    }
    console.log('-----------------------------------------');
  }
};
